*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZTEST_V
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZTEST_V             .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZTEST_V'
         EXPORTING
              FCODE          = READ
...
CALL FUNCTION 'VIEWPROC_ZTEST_V'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZTEST_V-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZTEST_V_EXTRACT
TOTAL = ZTEST_V_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZTEST_V-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZTEST_V'
...
TOTAL = ZTEST_V_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZTEST_V-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZTEST_V-UPD_FLAG NE SPACE.
